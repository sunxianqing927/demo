...
import sys
sys.path.append("..")
from common.ding_ding_send_msg import *
from common.remote_tool import *
from datetime import * 
from time import * 

        
dwzq=RemoteToolMgr.GetDefaultUsr()
...
def CheckServersLive():
    check_servers=["ClearServer","BaseDataServer","SearchServer","QueryServer"]
...
def CheckClearServer():
    global check_time
    if datetime.now()>check_time[0]:
        log_name=GetTodayLogName("/root/simulatetrade/ClearServer/log/ClearServer_DWZX_");
        if log_name=="":
            SendMsgToDingDing("清盘服务:错误,获取日志文件失败")
            return False
                
        grep_cmd = datetime.now().strftime("grep '%Y-%m-%d.*Clear Server success cleartimer'")
        hh=dwzq.ssh_cmd.Exec_Commend("cat "+log_name+" | " +grep_cmd+" | "+ "tail -1" )
        if hh[0] and len(hh[1]):
            SendMsgToDingDing("清盘服务:成功,"+hh[1][0])
            check_time[0]+=timedelta(days=1)
            check_time[1]+=timedelta(days=1)
            return True
        else:
            SendMsgToDingDing("清盘服务:失败")
